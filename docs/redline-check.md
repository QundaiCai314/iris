...
| D1 | 措辞红线机制 | 生成文案禁用承诺词 | iris/core/decision.py：BANNED_WORDS + _check_copy（运行时 raise）；test_decision 对全部条件句断言无禁词 | 🟢 | — |
...
| E2 | 页面免责 | 概率口径与假设可见 | iris/web/static/index.html footer：合成数据（截至 2026-09-03）、校准口径、A1-A7 假设可调、不承诺未来 | 🟢 | — |
...
## 结论
- 绿 14 项；黄 2 项（A3/A4，均为校准样本与偏差的如实披露，附研究信号）；豁免 1 项（C2，无新品演示场景）。
