...
- behavior_hints（M10，行为提示层）: [ {rule: promo_halo|high_percentile_rally|rerun_anxiety|fresh_card, text, ref} ]（确定性规则、不改裁决数学；rerun_anxiety 由 /api/recompute 传会话重算次数触发，fresh_card 仅首次出卡触发；文案过 BANNED_WORDS 红线）
